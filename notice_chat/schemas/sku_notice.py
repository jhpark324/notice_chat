...
class SkuNoticeBase(BaseModel):
    ...
    category: str = Field(..., max_length=50, description="공지 분류(학사/장학/일반 등)")
    author_org: str | None = Field(default=None, max_length=100, description="작성 부서 또는 작성자")
    posted_date: date | None = Field(default=None, description="공지 게시일")
    status: str | None = Field(default=None, max_length=20, description="진행 상태(진행중/진행완료/없음)")
...
class SkuNoticeCreate(SkuNoticeBase):
    """공지 생성/업서트 입력 스키마."""
...
class SkuNoticeUpdate(SkuNoticeBase):
    ...
    source_notice_id: int | None = Field(default=None, ge=1, description="학교 공지 원본 ID")
    detail_url: str | None = Field(default=None, max_length=500, description="공지 상세 페이지 URL")
    title: str | None = Field(default=None, max_length=500, description="공지 제목")
    category: str | None = Field(default=None, max_length=50, description="공지 분류(학사/장학/일반 등)")
    author_org: str | None = Field(default=None, max_length=100, description="작성 부서 또는 작성자")
    posted_date: date | None = Field(default=None, description="공지 게시일")
    status: str | None = Field(default=None, max_length=20, description="진행 상태(진행중/진행완료/없음)")
    list_number: int | None = Field(default=None, description="목록 번호")
    ...
    attachments: list[dict[str, Any]] | None = Field(default=None, description="첨부파일 목록(JSON)")


class SkuNoticeRead(SkuNoticeBase):
    """공지 조회 응답 스키마."""
